...
@Component({
  ...
  styles: [`
    ...
    .status-healthy, .status-passing, .status-succeeded, .status-active, .status-available, .status-certified { --mdc-chip-elevated-container-color: #e8f5e9; --mdc-chip-label-text-color: #2e7d32; }
    .status-degraded, .status-warning, .status-running, .status-in_review, .status-paused { --mdc-chip-elevated-container-color: #fff3e0; --mdc-chip-label-text-color: #e65100; }
